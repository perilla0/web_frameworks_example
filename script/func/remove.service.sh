...
PROJECT_DIR=`pwd`
REMOVE_TARGET_DIR=${PROJECT_DIR}/src/${DC_SERRVICE_NAME}

echo '#-----------------------------------------------------------------------------'
echo '# [サービス名]: ' \"${DC_SERRVICE_NAME}\" 'Docker Compose サービスを削除します'
echo '# [削除対象src] ' ${REMOVE_TARGET_DIR}
echo '#-----------------------------------------------------------------------------'
...
if [ -d ${REMOVE_TARGET_DIR} ]; then
    sudo rm -rf src/${DC_SERRVICE_NAME}
